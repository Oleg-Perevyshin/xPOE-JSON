...
8. **Без `strtod`/`sprintf`** — разбор и печать чисел не зависят от текущей locale процесса (у `strtod`
   разделитель дробной части — `.` только в locale `"C"`) и не тянут `stdio.h`/`stdlib.h`
9. **`-0.0` сохраняет знак** — проверка через `signbit()`, а не `val < 0` (по IEEE 754 `-0.0 < 0` ложно)
10. **`JSON_SortObject` без рекурсии** — итеративная сортировка слиянием (bottom-up, по образцу
