...
static bool parse_string(JSON* item, const char* input, size_t max_len,
                         size_t* out_offset, JSON_Context* ctx) {
...
/* Парсинг значения */
static bool parse_value(JSON* item, const char* input, size_t len,
                        size_t* out_offset, JSON_Context* ctx, size_t depth) {
  ...
  if (input[0] == '[') {
    ...
    do {
      ...
      size_t child_off = 0;
      if (!parse_value(new_node, input + cur, len - cur, &child_off, ctx,
                       depth + 1))
        return false;
      cur += child_off;
      if (!head) {
  ...
  if (input[0] == '{') {
    ...
    do {
      ...
      size_t key_off = 0;
      if (!parse_string(new_node, input + cur, len - cur, &key_off, ctx))
        return false;
      ...
      size_t val_off = 0;
      if (!parse_value(new_node, input + cur, len - cur, &val_off, ctx,
                       depth + 1))
        return false;
      cur += val_off;
...
/* ********************************************************** */
/* Сериализация */
static bool print_value(const JSON* item, char* buf, size_t size, size_t* pos,
                        size_t depth) {
  if (depth > JSON_MAX_DEPTH || !item || *pos >= size) return false;
    ...
    case JSON_Number: {
      ...
      if (isnan(val)) {
        ...
        *pos += 5;
        ...
        char tmp[32];
        int len;
        if (val > -9.22e18 && val < 9.22e18 &&
            fabs(val - (double)(long long)val) < 1e-9) {
          if (val >= 0 && val <= 4294967295.0)
            len = sprintf(tmp, "%lu", (unsigned long)val);
          else if (val >= -2147483648.0 && val <= 2147483647.0)
            len = sprintf(tmp, "%d", (int)val);
          else
            len = ll_to_str(tmp, (long long)val);
        } else {
          /* Знак храним отдельно: у int_part=0 (|val|<1) знака нет — иначе
           * "-0.5" печаталось бы как "0.5" */
          bool negative = val < 0;
          double aval = negative ? -val : val;
          int int_part = (int)aval;
          double frac = aval - int_part;
          int frac_part = (int)(frac * 1000000.0 + 0.5);
          if (frac_part >= 1000000) { /* округление перенесло дробную часть в
                                         целую (напр. 0.9999999) */
            frac_part -= 1000000;
            int_part++;
          }
          if (frac_part == 0) {
            len = sprintf(tmp, negative ? "-%d" : "%d", int_part);
          } else {
            /* Обрезаем хвостовые нули строкой, не int-делением */
            char frac_str[16]; /* frac_part математически всегда 0..999999, но
                                  компилятор судит по типу int — берём с запасом
                                */
            sprintf(frac_str, "%06d", frac_part);
            int flen = 6;
            while (flen > 1 && frac_str[flen - 1] == '0') flen--;
            frac_str[flen] = '\0';
            len =
                sprintf(tmp, negative ? "-%d.%s" : "%d.%s", int_part, frac_str);
          }
        }
    ...
    case JSON_String: {
      ...
      while (p && *p) {
        ...
        if (c == '"' || c == '\\') {
          ...
          if (*pos + 6 > size) return false;
          int w = snprintf(buf + *pos, 7, "\\u%04x", c);
          *pos += w;
        } else {
          if (*pos + 1 > size) return false;
          buf[(*pos)++] = c;
...
bool JSON_AddItemToArray(JSON_Context* ctx, JSON* array, JSON* item) {
  (void)ctx;
  if (!array || !item || (array->type & 0xFF) != JSON_Array || item->next)
    return false;
...
bool JSON_AddItemToObject(JSON_Context* ctx, JSON* object, const char* string,
                          JSON* item) {
  if (!ctx || !object || !string || !item ||
      (object->type & 0xFF) != JSON_Object || item->next)
    return false;
...
JSON* JSON_AddBoolToObject(JSON_Context* ctx, JSON* obj, const char* k,
                           bool v) {
  size_t saved_used = ctx->used;
...
JSON* JSON_AddNumberToObject(JSON_Context* ctx, JSON* obj, const char* k,
                             double v) {
  size_t saved_used = ctx->used;
...
JSON* JSON_AddStringToObject(JSON_Context* ctx, JSON* obj, const char* k,
                             const char* v) {
  size_t saved_used = ctx->used;
...
static JSON* dup_rec(JSON_Context* ctx, const JSON* src, size_t d) {
  if (!src || d > JSON_MAX_DEPTH) return NULL;
  JSON* n = alloc_node(ctx);
  if (!n) return NULL;
  n->type = src->type;
...
JSON* JSON_Duplicate(JSON_Context* ctx, const JSON* item) {
  return dup_rec(ctx, item, 0);
}

/* ********************************************************** */
/* Сортировка */
static JSON* merge(JSON* a, JSON* b) {
  if (!a) return b;
  if (!b) return a;
  if (strcmp(a->key_name, b->key_name) <= 0) {
...
static JSON* sort(JSON* h) {
  ...
  JSON* sec = s->next;
  s->next = NULL;
  if (sec) sec->prev = NULL;
  return merge(sort(h), sort(sec));
}

void JSON_SortObject(JSON_Context* ctx, JSON* obj) {
  (void)ctx;
  if (obj && (obj->type & 0xFF) == JSON_Object) obj->child = sort(obj->child);
}
